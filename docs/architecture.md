...
| 模块 | 状态 | 版本 | 说明 |
|------|------|------|------|
| 环境管理 | ✅ 已完成 | v0.2.0 | Base 环境 + 多环境管理 |
| Jupyter 集成 | ✅ 已完成 | v0.2.0 | Base 环境 JupyterLab + 全局内核存储 |
| 前端界面 | ✅ 已完成 | v0.1.0 | React + TypeScript |
| 项目管理 | ⏳ 计划中 | v0.3.0 | 多项目、导入导出 |
| 安全防护 | ⏳ 计划中 | v0.4.0 | 防炸盾、资源限制 |
...
应用初始化 (initialize_app)
    ├─ Step 1: 确保 base 环境存在 (~/.pyforge/base/)
    │   ├─ 不存在 → 创建并安装 JupyterLab
    │   └─ 已存在 → 跳过
    ├─ Step 2: 确保 default 环境存在 (~/.pyforge/envs/default/)
    │   ├─ 不存在 → 创建虚拟环境
...
- **路径隔离**：Jupyter 根目录限制在 ~/.pyforge/projects/
